...
const workingDir = new WorkingDirectory(config.workDir, config.claudeDesktopMode);
...
for (const spacePath of config.spacePaths) {
    try {
        const endpoint = await EndpointWrapper.createEndpoint(spacePath, workingDir);
        ...
        tools: [
            {
                ...
                description: "A list of available file and resources. " +
...
server.setRequestHandler(ListPromptsRequestSchema, async () => {
    return {
        prompts: [
            ...
            ...Array.from(endpoints.values()).map((endpoint) => endpoint.promptDefinition()),
        ],
    };
});
